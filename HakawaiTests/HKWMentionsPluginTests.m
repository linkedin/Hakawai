...
describe(@"prepend control character - MENTIONS PLUGIN V2", ^{
    ...
    beforeEach(^{
        HKWTextView.enableMentionsPluginV2 = YES;
        HKWTextView.enableControlCharactersToPrepend = YES;
        textView = [[HKWTextView alloc] initWithFrame:CGRectMake(0, 0, 100, 100)];
        mentionsPlugin = [HKWMentionsPluginV2 mentionsPluginWithChooserMode:HKWMentionsChooserPositionModeCustomLockTopArrowPointingUp
    ...
    it(@"prepend Japanese ＠", ^{
        mentionsPlugin.controlCharactersToPrepend = HKWExternalMentionConstants.atSymbols;
        textView.text = @"＠リンクトイン";
        HKWMentionsAttribute *attribute = [HKWMentionsAttribute mentionWithText:@"リンクトイン アメリカ" identifier:@"2"];
        [mentionsPlugin createMention:attribute cursorLocation:textView.text.length - 1];

        expect(textView.text).to.equal(@"＠リンクトイン アメリカ");
        expect(attribute.mentionText).to.equal(@"＠リンクトイン アメリカ");
    });
    ...
    it(@"do not prepand character when controlCharactersToPrepend is empty", ^{
    ...
    it(@"ignore control character with attribute", ^{
        mentionsPlugin.controlCharactersToPrepend = [NSCharacterSet characterSetWithCharactersInString:@"@#"];
        textView.text = @"@First";
        HKWMentionsAttribute *attribute = [HKWMentionsAttribute mentionWithText:@"FirstName LastName" identifier:@"5"];
        [mentionsPlugin createMention:attribute cursorLocation:textView.text.length - 1];
    ...
    it(@"highlight mention text with control character prepended", ^{
        ...
        id highlightedAttribute = [textView.attributedText attribute:HKWRoundedRectBackgroundAttributeName atIndex:attribute.mentionText.length/2 longestEffectiveRange:&highlightedRange inRange:NSMakeRange(0, textView.attributedText.length)];
    ...
    it(@"can find control character when text length exceeds MAX_MENTION_QUERY_LENGTH", ^{
        ...
        expect([mentionsPlugin mostRecentControlCharacterLocationInText:textView.text locationOffsetInOriginalText:textView.text.length - MAX_MENTION_QUERY_LENGTH]).to.equal(textView.text.length - 1);
    });
});
